...
export function KanbanBoard() {
  ...
  // Get valid column IDs for SortableContext (only columns with valid IDs can be dragged)
  const columnsId = useMemo(() => {
    return columns
      .map((col) => col.id)
      .filter((id): id is UniqueIdentifier => Boolean(id) && id !== '');
  }, [columns]);
      ...
      <BoardContainer>
        <SortableContext items={columnsId}>
          {columns?.map((col, index) => {
            // Ensure unique key even if col.id is empty or duplicate
            const columnKey = col.id && col.id !== '' 
              ? `${String(col.id)}-${index}` 
              : `column-${index}-${col.title || 'untitled'}`;
            return (
